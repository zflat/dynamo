...
describe "dynamo archive" do
  ...
  it "archive is successful" do
    prepare_examples("profile") do |dir|
      out = dynamo("archive " + dir + " --overwrite")
  ...
  it "archives to output file" do
    prepare_examples("profile") do |dir|
      out = dynamo("archive " + dir + " --output " + dst.path)
  ...
  it "archives an dynamo.json file" do
    prepare_examples("profile") do |dir|
      out = dynamo("archive " + dir + " --overwrite")

      _(out.stderr).must_equal ""
      t = Zlib::GzipReader.open(auto_dst)
      _(Gem::Package::TarReader.new(t).entries.map(&:header).map(&:name)).must_include "dynamo.json"
      assert_exit_code 0, out
    end
  end
  ...
  it "archive on invalid archive" do
    Dir.tmpdir do |target_dir|
      out = dynamo("archive #{target_dir} --output " + dst.path)

      _(out.stderr).must_include "Don't understand dynamo profile in \"#{target_dir}\""
  ...
  it "creates valid tar.gz archives" do
    prepare_examples("profile") do |dir|
      out = dynamo("archive " + dir + " --output " + dst.path + " --tar")

      _(out.stderr).must_equal ""
      _(out.stdout).must_include "Generate archive " + dst.path
      t = Zlib::GzipReader.open(dst.path)
      _(Gem::Package::TarReader.new(t).entries.map(&:header).map(&:name)).must_include "dynamo.yml"
      assert_exit_code 0, out
    end
  end
  ...
  it "creates valid zip archives" do
    prepare_examples("profile") do |dir|
      out = dynamo("archive " + dir + " --output " + dst.path + " --zip")

      _(out.stderr).must_equal ""
      _(out.stdout).must_include "Generate archive " + dst.path
      _(Zip::File.new(dst.path).entries.map(&:name)).must_include "dynamo.yml"
      assert_exit_code 0, out
    end
  end
  ...
  it "vendors dependencies by default" do
    prepare_examples("meta-profile") do |dir|
      ...
      _(files).must_include "dynamo.lock"
  ...
  it "can archive a profile with required inputs" do
    ...
    Dir.mktmpdir do |tmpdir|
      ...
      out = dynamo("archive " + tmpdir + " --output " + dst.path)
