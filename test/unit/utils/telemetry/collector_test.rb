...
class TestTelemetryCollector < Minitest::Test
  ...
  def test_collector_singleton
    assert_equal Dynamo::Telemetry::Collector.instance, @collector
  end

  def test_add_data_series
    assert_empty @collector.list_data_series
    assert @collector.add_data_series(Dynamo::Telemetry::DataSeries.new("/resource/File"))
    refute_empty @collector.list_data_series
  end

  def test_list_data_series
    assert_empty @collector.list_data_series
    @collector.add_data_series(Dynamo::Telemetry::DataSeries.new("/resource/File"))
    @collector.add_data_series(Dynamo::Telemetry::DataSeries.new(:deprecation_group))
    ...
    assert_kind_of Dynamo::Telemetry::DataSeries, @collector.list_data_series.first
  end

  def test_find_or_create_data_series
    dg = @collector.find_or_create_data_series(:deprecation_group)
    assert_kind_of Dynamo::Telemetry::DataSeries, dg
  ...
  def test_reset_singleton
    data_series = Dynamo::Telemetry::DataSeries.new("/resource/File")
  ...
  def test_telemetry_disabled
    @collector.load_config(Dynamo::Config.mock("enable_telemetry" => false))
